<template>
  ...
  <div class="gap-2 flex flex-wrap pt-2 max-h-40 overflow-auto">
    <div
      ...
      :class="{'bg-royal-purple-700 text-white': model.includes(item.id)}"
...
import { Brand } from '@/custom_types';
...
function changeValue(item: Brand) {
...
const filteredList = computed(() => {
    ...
    let sortedAndFilteredList = sortObjectsAlphabetically<Brand>(props.list as Brand[], "name").filter((listElement) => regex.test(listElement.name));
